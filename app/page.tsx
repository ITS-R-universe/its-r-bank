...
export default function BankHome() {
  ...
  const steps = [
    { n: '1', title: 'Send via Nayapay', desc: "Send PKR to founder's Nayapay account" },
    { n: '2', title: 'Submit Request', desc: 'Fill deposit form with Nayapay reference' },
    { n: '3', title: 'Admin Confirms', desc: 'Founder verifies the payment' },
    { n: '4', title: 'Coins Credited', desc: 'ITSR Coins added to your wallet' },
  ]
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '3rem 1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '1rem' }}>
            {steps.map(s => (
              <div key={s.n} style={{ textAlign: 'center', padding: '1rem' }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--gold)', color: '#000', fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 0.75rem', fontSize: '1.1rem' }}>{s.n}</div>
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '2rem' }}>
          <h2 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1rem', color: 'var(--gold)' }}>⚠️ Important Rules</h2>
        ...
        <div style={{ textAlign: 'center' }}>
          <a href="https://its-r-passport.vercel.app/login?redirect=https://its-r-bank.vercel.app/wallet"
            style={{ display: 'inline-block', padding: '1rem 2rem', background: 'var(--gold)', color: '#000', fontWeight: 700, borderRadius: '0.75rem', fontSize: '1rem' }}>
